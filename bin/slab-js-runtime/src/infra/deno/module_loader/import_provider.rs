use deno_core::{ModuleSource, ModuleSpecifier, error::ModuleLoaderError};
...
pub trait ImportProvider {
    ...
    ///
    /// Override to supply custom module sources. The default returns `None` so the
    /// loader falls back to its standard resolution behavior.
    ///
    /// # Arguments
