...
//!
//! Pure conversion layer from slab-owned agent domain events
//! (`slab_agent::AgentEventKind` / `slab_app_core::infra::agent::event_hub::AgentEventEnvelope`)
//! into the canonical OpenAI Responses wire types (`slab_proto::openai::models`).
//!
//! Boundary rules:
//! - The adapter NEVER calls `AgentService` and never touches `tokio`/`axum`.
...
// Transitional: the adapter is wired into the HTTP layer incrementally. These
// allows are removed once the handler rewrite calls every entry point.
#![allow(dead_code)]
...
use slab_proto::openai::{
    ApplyPatchCallStatus, ApplyPatchCreateFileOperation, ApplyPatchDeleteFileOperation,
    ApplyPatchOperation, ApplyPatchToolCall, ApplyPatchToolCallType, ApplyPatchUpdateFileOperation,
    CodeCompletedType, CodeDeltaType, CodeDoneType, CodeInProgressType, CodeInterpretingType,
    CompactionBody, CompactionBodyType, CreateFileOperationType, CustomToolCall,
    CustomToolCallInputDeltaType, CustomToolCallType, DeleteFileOperationType,
    FunctionCallOutputStatusEnum, FunctionCallStatus, FunctionShellAction, FunctionShellCall,
    FunctionShellCallOutput, FunctionShellCallOutputStatusEnum, FunctionShellCallOutputType,
    FunctionShellCallStatus, FunctionShellCallType, LocalShellExecAction, LocalShellExecActionType,
    LocalShellToolCall, LocalShellToolCallStatus, LocalShellToolCallType, McpApprovalRequest,
    McpApprovalRequestType, McpCallArgumentsDeltaType, McpCallArgumentsDoneType,
    McpCallCompletedType, McpCallInProgressType, McpListTools, McpListToolsCompletedType,
    McpListToolsInProgressType, McpListToolsTool, McpListToolsType, McpToolCall, McpToolCallStatus,
    McpToolCallType, ResponseCodeInterpreterCallCodeDeltaEvent,
    ...
    ResponseFileSearchCallCompletedEvent, ResponseFileSearchCallCompletedEventType,
    ResponseFileSearchCallInProgressEvent, ResponseFileSearchCallInProgressEventType,
    ResponseFileSearchCallSearchingEvent, ResponseFileSearchCallSearchingEventType,
    ResponseMcpCallArgumentsDeltaEvent, ResponseMcpCallArgumentsDoneEvent,
    ResponseMcpCallCompletedEvent, ResponseMcpCallInProgressEvent,
    ResponseMcpListToolsCompletedEvent, ResponseMcpListToolsInProgressEvent,
    ResponseShellCallCommandAddedEvent, ResponseShellCallCommandDeltaEvent,
    ResponseShellCallCommandDoneEvent, ResponseShellCallOutputContentDeltaEvent,
    ResponseShellCallOutputContentDoneEvent, ResponseWebSearchCallCompletedEvent,
    ResponseWebSearchCallInProgressEvent, ResponseWebSearchCallSearchingEvent,
    ShellCallCommandAddedType, ShellCallCommandDeltaType, ShellCallCommandDoneType,
    ShellCallOutputContentDelta, ShellCallOutputContentDeltaType, ShellCallOutputContentDoneType,
    ToolSearchCall, ToolSearchCallType, ToolSearchExecutionType, ToolSearchOutput,
    ToolSearchOutputType, UpdateFileOperationType, WebSearchCompletedType, WebSearchInProgressType,
    WebSearchSearchingType,
};
use slab_proto::openai::{
    CommonOutputType, ContentPartAddedType, ContentPartDoneType, FuncArgsDeltaType,
    FuncArgsDoneType, FunctionToolCall, FunctionToolCallStatus, MessagePhase, ModelIdsResponses,
    OutputContent, OutputItem, OutputItemAddedType, OutputItemDoneType, OutputMessage,
    OutputMessageContent, OutputMessageRole, OutputTextContent, Reasoning, ReasoningItem,
    ReasoningItemType, ReasoningSummaryPartAddedType, ReasoningSummaryPartDoneType,
    ReasoningSummaryTextDeltaType, ReasoningSummaryTextDoneType, Response, ResponseCompletedEvent,
    ResponseCompletedType, ResponseContentPartAddedEvent, ResponseContentPartDoneEvent,
    ResponseCreatedEvent, ResponseCreatedType, ResponseCustomToolCallInputDeltaEvent,
    ResponseErrorEvent, ResponseFailedEvent, ResponseFailedType,
    ResponseFunctionCallArgumentsDeltaEvent, ResponseFunctionCallArgumentsDoneEvent,
    ResponseInProgressEvent, ResponseInProgressType, ResponseObject, ResponseOutputItemAddedEvent,
    ResponseOutputItemDoneEvent, ResponsePromptCacheRetention,
    ResponseReasoningSummaryPartAddedEvent, ResponseReasoningSummaryPartAddedEventPart,
    ResponseReasoningSummaryPartDoneEvent, ResponseReasoningSummaryPartDoneEventPart,
    ResponseReasoningSummaryTextDeltaEvent, ResponseReasoningSummaryTextDoneEvent, ResponseStatus,
    ResponseTextDeltaEvent, ResponseTextDoneEvent, ResponseTextParam, ResponseTruncation,
    ResponseUsage, ResponsesServerEvent, ServiceTier, Status, SummaryTextContent,
    SummaryTextContentType, SummaryTextType, TextDeltaType, TextDoneType, Tool, ToolChoiceParam,
};

/// Input the converter needs from the handler. Decouples from `AgentService`
/// (which is a concrete struct, not a trait) so unit tests can construct this
/// directly without mocking the service.
///
/// Every field beyond the core identifiers is an optional echoed request-config
/// value: the handler fills in whichever fields the upstream request carried,
/// and `build_response` only emits the ones that are `Some`.
#[derive(Default)]
pub struct AdapterInput<'a> {
    /// `resp_<...>` identifier (== the slab thread id).
    ...
    /// Ordered, already-filtered-to-one-response slab event envelopes.
    pub envelopes: &'a [AgentEventEnvelope],
    /// Whether to run the model response in the background.
    pub background: Option<bool>,
    /// Billing attribution echoed on the response (e.g. `{ "payer": "developer" }`).
    ...
    for envelope in input.envelopes {
        ...
        match event {
            ...
            AgentEventKind::ResponseToolSearchOutputDone {
                ...
                // `tools` arrives as opaque JSON from slab-agent; now that the
                // canonical `Tool` type round-trips (internally tagged, no inner
                // `type` field), parse each definition into the typed shape and
            ...
            AgentEventKind::ResponseMcpListToolsDone {
                ...
                // the typed `McpListToolsTool` shape, dropping any that do not fit.
            ...
            AgentEventKind::ResponseWebSearchCallDone { item_id, action, .. } => {
                // The action JSON (search / open_page / find_in_page) round-trips
                // through the typed `WebSearchToolCallAction` (internally tagged,
                // snake_case variants). Parse failure falls back to a search
                // skeleton so the output item still serializes.
            ...
            AgentEventKind::ResponseShellCallOutputContentDone {
                ...
                // Each opaque output element ({stdout, stderr, outcome:{type,
                // exit_code}}) is parsed into the typed
                // `FunctionShellCallOutputContent`. `outputs` is opaque JSON
                // because slab-agent does not depend on slab-proto.
...
fn parse_mcp_status(raw: &str) -> Option<McpToolCallStatus> {
    match raw {
...
fn parse_phase(raw: &str) -> MessagePhase {
    match raw {
...
/// `None` when `environment_type` is absent (the field is omitted entirely);
/// otherwise `{type:"local"}` or `{type:"container_reference",container_id}`.
...
/// Parse one opaque shell-output element (`{stdout, stderr, outcome}`) from the
/// slab `ResponseShellCallOutputContentDone.outputs` array into the typed
/// [`slab_proto::openai::FunctionShellCallOutputContent`]. Returns `None` when
/// the element is not a JSON object. The `outcome` discriminator is
/// `exit` (with `exit_code`) or `timeout`; an unrecognized outcome falls back to
/// a zero-code exit so the content still serializes.
fn parse_shell_output_content(
    v: &serde_json::Value,
...
// ---------------------------------------------------------------------------
// Streaming API (Phase 2 — bodies filled in by the adapter owner).
//
// The PUBLIC SIGNATURES here are FROZEN so the HTTP-layer rewrite can code
// against them in parallel. The bodies are stubs until the streaming state
// machine lands.
// ---------------------------------------------------------------------------
