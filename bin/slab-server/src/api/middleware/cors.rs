...
pub fn cors_layer(state: Arc<AppState>) -> CorsLayer {
    ...
    let origins = resolve_allowed_origins(&settings.server.cors.allowed_origins);
...
fn resolve_allowed_origins(configured_origins: &[String]) -> Vec<HeaderValue> {
    if configured_origins.is_empty() {
        return default_allowed_origins();
    }

    parse_configured_origins(configured_origins).unwrap_or_else(|| {
            warn!(
                configured = ?configured_origins,
                "server.cors.allowed_origins resolved to an empty allowlist; rejecting cross-origin requests"
            );
            Vec::new()
        })
}

fn parse_configured_origins(configured_origins: &[String]) -> Option<Vec<HeaderValue>> {
...
mod tests {
    ...
    async fn preflight_allow_origin(configured_origins: &[String], origin: &str) -> Option<String> {
        let app = Router::new()
    ...
    async fn default_allowlist_allows_vite_dev_origin() {
        let allow_origin = preflight_allow_origin(&[], "http://localhost:1420").await;
    ...
    async fn default_allowlist_rejects_unrelated_origin() {
        let allow_origin = preflight_allow_origin(&[], "https://example.com").await;
    ...
    async fn explicit_cors_origins_override_default_allowlist() {
        let explicit_allow_origin =
            preflight_allow_origin(&["https://example.com".to_owned()], "https://example.com")
                .await;
        let default_allow_origin =
            preflight_allow_origin(&["https://example.com".to_owned()], "http://localhost:1420")
                .await;
    ...
    async fn empty_configured_cors_origin_list_rejects_cross_origin_requests() {
        let allow_origin = preflight_allow_origin(&[" ".to_owned()], "http://localhost:1420").await;
