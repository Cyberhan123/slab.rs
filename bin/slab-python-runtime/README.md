...
`runtime.python.entry` points to the Python module entry. In development it is
a `.py` file inside the plugin package. Packaged plugins may also declare
`runtime.python.bundle`, a `.slabpy` bundle generated by the plugin packer and
loaded through the runtime VFS. The exported function receives a JSON value and
...
The first version supports plugin `.py` files, `.slabpy` bundles containing
pure Python modules, and pure Python stdlib modules. It does not support native
extensions.
