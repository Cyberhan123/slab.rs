...
use slab_types::{
    Capability, DriverDescriptor, DriverHints as DispatchHints, ModelSource, ModelSourceKind,
    ModelSpec,
};

use super::plan::ResolvedDriver;
...
mod tests {
    ...
    use super::*;
    use slab_types::{Capability, DriverLoadStyle, ModelFamily, ModelSource};
    ...
    fn resolver_filters_by_family_capability_and_source() {
        ...
        let spec = ModelSpec::new(
            ModelFamily::Llama,
            Capability::TextGeneration,
            ModelSource::LocalPath { path: PathBuf::from("model.gguf") },
        );
    ...
    fn resolver_returns_no_viable_driver_when_none_match() {
        ...
        let spec = ModelSpec::new(
            ...
            ModelSource::LocalPath { path: PathBuf::from("model.bin") },
        );
    ...
    fn resolver_rejects_streaming_when_driver_cannot_stream() {
        ...
        let spec = ModelSpec::new(
            ModelFamily::Onnx,
            Capability::TextGeneration,
            ModelSource::LocalPath { path: PathBuf::from("model.onnx") },
        );
