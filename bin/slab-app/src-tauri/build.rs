...
fn main() {
    ...
    let skip_tauri_sidecar_sync =
        env::var("SLAB_SKIP_TAURI_SIDECAR_SYNC").is_ok_and(|value| value == "1");
    let skip_bundled_plugin_sync =
        env::var("SLAB_SKIP_BUNDLED_PLUGIN_SYNC").is_ok_and(|value| value == "1");
    ...
    println!("cargo:rerun-if-env-changed=SLAB_SKIP_TAURI_SIDECAR_SYNC");
    println!("cargo:rerun-if-env-changed=SLAB_SKIP_BUNDLED_PLUGIN_SYNC");

    if !skip_tauri_sidecar_sync {
        ...
        sync_tauri_sidecar("slab-python-runtime", &target, &workspace_target_dir, &manifest_dir)
            .expect("failed to sync slab-python-runtime sidecar");
    }
    if !skip_bundled_plugin_sync {
