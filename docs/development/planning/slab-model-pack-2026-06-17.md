...
> **代码优先闭环（2026-06-19）**：v3 model-pack 的引擎回退耗尽路径已按当前代码接入 runtime reliability 的 general envelope：生产路径返回 `AppCoreError::RuntimeFailure`，其 `AppCoreErrorData::runtime_failure("runtime_engine_exhausted", detail)` 中 `detail = { "model_id": ..., "attempts": [...] }`，`attempts[]` 复用 `RuntimeEngineAttemptError { engine, outcome, message }`。不再新增或发送 `ServerError::RuntimeEngineExhausted` / `AppCoreErrorData::RuntimeEngineExhausted` / `data.rollback` 特化信封。
...
RuntimeGateway::load_model                 runtime_gateway.rs:194-202
   ...
   → 耗尽则抛 AppCoreError::RuntimeFailure（runtime_code = "runtime_engine_exhausted"，detail 携带 model_id + attempts[]）
```
...
Selected ─dispatch(load_cmd[0])─▶ Loading ─ok─▶ Ready
                                   ...
                                   └─引擎致命─▶ LoadFailed ─fallback 非空?─是─▶ Fallback ─▶ Loading(下一)
                                                              │
                                                              └─否─▶ Exhausted ─▶ Error(500 runtime_engine_exhausted)
```
...
```jsonc
{
  ...
  "data": {
    ...
    "runtime_code": "runtime_engine_exhausted",
    "detail": {
      "model_id": "Qwen3.5-9B",
      "attempts": [
        { "engine": "ggml.llama",   "outcome": "memory_pressure", "message": "…" },
        { "engine": "candle.llama", "outcome": "backend_missing", "message": "…" }
      ]
    }
  }
}
...
- **文件**：
  ...
  - [domain/services/model/runtime.rs](../../../crates/slab-app-core/src/domain/services/model/runtime.rs)：`load_model_with_state` 实现 §4.7 状态机——按解析后的候选链派发，遇 §4.4 引擎致命分类记录 attempt 并重试，耗尽抛 `AppCoreError::RuntimeFailure`（`runtime_code = "runtime_engine_exhausted"`，`detail = { model_id, attempts[] }`）。
  - [supervisor.rs](../../../crates/slab-app-core/src/infra/runtime/supervisor.rs)：**不改结构**，仅在 :54-60 一带加 doc 注释澄清"引擎回退是网关职责、进程重启是监督者职责"。
  - [error.rs](../../../bin/slab-server/src/error.rs) + [app-core error.rs](../../../crates/slab-app-core/src/error.rs)：复用 `RuntimeFailure` general envelope，`ErrorResponse.data.runtime_code` 与 agent `Error.code` 均为 `runtime_engine_exhausted`，不再发送 `data.rollback` 特化信封。
- **关闭**：**F-Stack-3**（跨边界保结构）。
...
| **F-Stack-3** gRPC 错误跨边界损失结构 | `RuntimeFailure` general envelope + `runtime_engine_exhausted` detail attempts | Phase 2 |
