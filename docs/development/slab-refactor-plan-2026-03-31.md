...
- WP3 `P0` Chat 契约收口：域层请求拆成 `CommonChatParams`、`LocalChatParams`、`CloudChatParams`；API 层继续接受现有字段，但在模型路由确定后做显式校验，本地拒绝 `reasoning_effort/verbosity`，云端拒绝原始 `gbnf`，`response_format/json_schema` 统一映射为 structured output；所有“不生效参数”统一返回 `400 unsupported_chat_parameter`，不再静默忽略。
- WP4 `P0` Chat 能力显式化：`GET /v1/chat/models` 增加 `capabilities` 字段，至少包含 `raw_gbnf`、`structured_output`、`reasoning_controls`；前端聊天页按能力开关禁用不支持的控制项，不再靠失败后回退判断。
