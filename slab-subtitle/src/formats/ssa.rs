...
pub mod errors {
    ...
    pub enum ErrorKind {
        #[error(
            ".ssa/.ass file did not have a line beginning with `Format: ` in a `[Events]` section"
        )]
        ...
        #[error("the dialog at line {line_num} has incorrect number of fields")]
...
impl SsaFieldsInfo {
    ...
    fn new_from_fields_info_line(line_num: usize, s: String) -> Result<SsaFieldsInfo> {
        ...
        for (i, field_name) in split_iter.enumerate() {
            ...
            if trimmed == "Start" {
                if start_field_idx.is_some() {
                    return Err(Error::from(SsaDuplicateField { line_num, f: "Start" }));
                }
                start_field_idx = Some(i);
            } else if trimmed == "End" {
                if end_field_idx.is_some() {
                    return Err(Error::from(SsaDuplicateField { line_num, f: "End" }));
                }
                end_field_idx = Some(i);
            } else if trimmed == "Text" {
                if text_field_idx.is_some() {
                    return Err(Error::from(SsaDuplicateField { line_num, f: "Text" }));
                }
        ...
        let text_field_idx2 =
            text_field_idx.ok_or_else(|| Error::from(SsaMissingField { line_num, f: "Text" }))?;
        if text_field_idx2 != num_fields - 1 {
            return Err(Error::from(SsaTextFieldNotLast { line_num }));
        }

        Ok(SsaFieldsInfo {
            start_field_idx: start_field_idx
                .ok_or_else(|| Error::from(SsaMissingField { line_num, f: "Start" }))?,
...
impl SsaFile {
    ...
    /// "Dialogue: 1,0:22:43.52,0:22:46.22,ED-Romaji,,0,0,0,,{\fad(150,150)\blur0.5\bord1}some text"
    /// ```
    fn parse_dialog_line(
        line_num: usize,
        line: &str,
        fields_info: &SsaFieldsInfo,
        ...
        match parts_res {
            ...
            Ok((parts, _)) => Ok(parts?),
            Err(e) => {
                Err(SsaDialogLineParseError { line_num, msg: parse_error_to_string(e) }.into())
            }
        }
    }
    ...
    fn parse_fields(
        ...
        let extract_file_parts_closure =
            |(i, (field, sep_char)): (_, (String, char))| -> Result<Vec<SsaFilePart>> {
    ...
    fn parse_timepoint(line_num: usize, s: &str) -> Result<TimePoint> {
        ...
        match parse_res {
            Ok(res) => Ok(res.0),
            Err(e) => {
                Err(SsaWrongTimepointFormat { line_num, string: parse_error_to_string(e) }.into())
            }
        }
    }
}
...
impl SsaFile {
    ...
    fn get_subtitle_entries_mut<'a>(
        &'a mut self,
    ) -> Vec<(&'a mut TimePoint, &'a mut TimePoint, &'a mut String)> {
        ...
        let timings: Vec<_> = {
            let filter_map_closure = |part: &'a mut SsaFilePart| -> Option<(
                &'a mut TimePoint,
                &'a mut TimePoint,
                &'a mut String,
            )> {
...
impl SubtitleFileInterface for SsaFile {
    fn get_subtitle_entries(&self) -> SubtitleParserResult<Vec<SubtitleEntry>> {
        ...
        let timings = new_file
            ...
            .into_iter()
            .map(|(start, end, text)| SubtitleEntry::new(TimeSpan::new(*start, *end), text.clone()))
            .collect();
