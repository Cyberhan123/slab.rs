...
pub(crate) fn validate_settings_schema_document(document: &Value) -> Result<(), AppCoreError> {
...
pub(crate) fn ensure_json_schema_is_valid(schema: &Value, label: &str) -> Result<(), AppCoreError> {
    jsonschema::draft202012::meta::validate(schema).map_err(|error| {
        AppCoreError::Internal(format!("{label} is not a valid JSON Schema: {error}"))
    })?;
    jsonschema::validator_for(schema)
        .map(|_| ())
        .map_err(|error| AppCoreError::Internal(format!("{label} failed to compile: {error}")))
}
