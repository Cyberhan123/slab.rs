...
pub fn ensure_model_config_dir(path: &Path) -> Result<(), AppCoreError> {
...
pub fn list_model_config_paths(dir: &Path) -> Result<Vec<PathBuf>, AppCoreError> {
...
pub fn read_model_config(path: &Path) -> Result<StoredModelConfig, AppCoreError> {
    ...
    serde_json::from_str(&raw).map_err(|error| {
        AppCoreError::BadRequest(format!("invalid model config file '{}': {error}", path.display()))
    })
}

pub fn write_model_config(dir: &Path, config: &StoredModelConfig) -> Result<PathBuf, AppCoreError> {
    ensure_model_config_dir(dir)?;
...
pub fn delete_model_config(dir: &Path, id: &str) -> Result<bool, AppCoreError> {
...
fn write_json_file<T: serde::Serialize>(path: &Path, value: &T) -> Result<(), AppCoreError> {
