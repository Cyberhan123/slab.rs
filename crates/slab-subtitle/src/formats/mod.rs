...
pub fn is_valid_extension_for_subtitle_format(
    ...
    match format {
        ...
        SubtitleFormat::SubStationAlpha => {
            extension == Some(OsStr::new("ssa")) || extension == Some(OsStr::new("ass"))
        }
...
/// will be returned (for simpler error handling).
pub fn get_subtitle_format_err(extension: Option<&OsStr>) -> Result<SubtitleFormat> {
    get_subtitle_format(extension).ok_or_else(|| ErrorKind::UnknownFileFormat.into())
}
...
///
/// See `parse_bytes`.
pub fn parse_str(format: SubtitleFormat, content: &str, _fps: f64) -> Result<SubtitleFile> {
    match format {
...
fn decode_bytes_to_string(content: &[u8], encoding: Option<&'static Encoding>) -> Result<String> {
    let det_encoding = match encoding {
        ...
        None => {
            ...
            Encoding::for_label_no_replacement(encoding_name.as_bytes())
                .ok_or(ErrorKind::EncodingDetectionError)?
        }
    };
...
pub fn parse_bytes(
    ...
    match format {
        SubtitleFormat::SubRip => {
            Ok(srt::SrtFile::parse(&decode_bytes_to_string(content, encoding)?)?.into())
        }
        SubtitleFormat::SubStationAlpha => {
            Ok(ssa::SsaFile::parse(&decode_bytes_to_string(content, encoding)?)?.into())
        }
        SubtitleFormat::VobSubIdx => {
            Ok(idx::IdxFile::parse(&decode_bytes_to_string(content, encoding)?)?.into())
        }
    }
}
