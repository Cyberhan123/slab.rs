...
async fn spawn_process_portable(
    ...
    Ok(SpawnedProcess { session: handle, stdout_rx, stderr_rx, exit_rx })
}
...
async fn spawn_process_preserving_fds(
    ...
    let handles = PtyHandles {
        _slave: None,
        _master: PtyMasterHandle::Opaque { raw_fd: master.as_raw_fd(), _handle: Box::new(master) },
    };
...
fn open_unix_pty(size: TerminalSize) -> Result<(File, File)> {
    ...
    let mut size =
        libc::winsize { ws_row: size.rows, ws_col: size.cols, ws_xpixel: 0, ws_ypixel: 0 };
    let winp = std::ptr::addr_of_mut!(size);

    let result = unsafe {
        libc::openpty(&mut master, &mut slave, std::ptr::null_mut(), std::ptr::null_mut(), winp)
    };
    if result != 0 {
