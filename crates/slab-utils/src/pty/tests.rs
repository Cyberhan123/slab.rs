...
{
    let SpawnedProcess { session, stdout_rx, stderr_rx, exit_rx } = spawned;
    (session, combine_output_receivers(stdout_rx, stderr_rx), exit_rx)
}
...
async fn wait_for_python_repl_ready_via_probe(
    ...
    while tokio::time::Instant::now() < deadline {
        writer.send(format!("print('{marker}'){newline}").into_bytes()).await?;
...
async fn wait_for_marker_pid(
    ...
    loop {
        ...
        while let Some(pos) = text[offset..].find(marker) {
            ...
            let digits_len =
                suffix.chars().take_while(char::is_ascii_digit).map(char::len_utf8).sum::<usize>();
            if digits_len == 0 {
...
async fn pty_python_repl_emits_output_and_exits() -> anyhow::Result<()> {
    ...
    let spawned =
        spawn_pty_process(&python, &args, Path::new("."), &env_map, &None, TerminalSize::default())
            .await?;
    ...
    assert!(text.contains("hello from pty"), "expected python output in PTY: {text:?}");
...
async fn pipe_process_round_trips_stdin() -> anyhow::Result<()> {
    ...
    let writer = session.writer_sender();
    let newline = if cfg!(windows) { "\r\n" } else { "\n" };
    writer.send(format!("roundtrip{newline}").into_bytes()).await?;
    drop(writer);
    session.close_stdin();
...
async fn pipe_process_detaches_from_parent_session() -> anyhow::Result<()> {
    ...
    assert_ne!(child_sid, parent_sid, "expected child to be detached from parent session");

    let exit_code = exit_rx.await.unwrap_or(-1);
    assert_eq!(exit_code, 0, "expected detached pipe process to exit cleanly");
...
async fn pipe_process_can_expose_split_stdout_and_stderr() -> anyhow::Result<()> {
    ...
    let SpawnedProcess { session: _session, stdout_rx, stderr_rx, exit_rx } = spawned;
...
async fn pty_preserving_inherited_fds_keeps_python_repl_running() -> anyhow::Result<()> {
    ...
    let mut env_map: HashMap<String, String> = std::env::vars().collect();
    env_map.insert("PRESERVED_FD".to_string(), preserved_fd.as_raw_fd().to_string());
...
async fn pty_spawn_with_inherited_fds_supports_resize() -> anyhow::Result<()> {
    ...
    let writer = session.writer_sender();
    let mut output =
        wait_for_output_contains(&mut output_rx, "start:31 101\r\n", /*timeout_ms*/ 5_000).await?;

    session.resize(TerminalSize { rows: 45, cols: 132 })?;
    writer.send(b"go\n".to_vec()).await?;
    session.close_stdin();
