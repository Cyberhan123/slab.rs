-- Drop the legacy agent conversation tables + agent_tool_calls audit
-- table + the rollout backfill state table.
--
-- After the rollout JSONL event-source refactor, the rollout true source is
-- the ONLY live writer/reader for
...
-- startup backfill that is itself being removed here (rollout is now the
-- only source, so there is nothing to backfill).
--
-- `rollout_session_index` is KEPT (its `backfill_status` / `line_count` columns
-- still back the list-path ghost-gate; the `backfill_status` name is now a
