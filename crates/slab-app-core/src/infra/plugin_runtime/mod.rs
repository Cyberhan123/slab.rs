...
use host_api::{authorize_slab_api_request, execute_plugin_api_request};
pub use process::resolve_js_runtime_exe;

const PLUGIN_CALL_TIMEOUT: Duration = Duration::from_secs(300);

#[derive(Clone)]
pub struct PluginJsRuntimeClient {
    inner: Arc<PluginJsRuntimeClientInner>,
}

struct PluginJsRuntimeClientInner {
    runtime_exe: std::path::PathBuf,
    api_base_url: String,
...
impl PluginJsRuntimeClient {
    pub fn new(
        ...
        Self {
            inner: Arc::new(PluginJsRuntimeClientInner {
                runtime_exe,
                api_base_url,
                event_bus,
    ...
    pub fn for_current_server(api_base_url: String, event_bus: PluginEventBus) -> Self {
        let runtime_exe = std::env::current_exe()
            .ok()
            .and_then(|server_exe| resolve_js_runtime_exe(&server_exe).ok())
            .unwrap_or_else(|| std::path::PathBuf::from("slab-js-runtime"));
        Self::new(runtime_exe, api_base_url, event_bus)
    }
    ...
    pub async fn call(
        ...
        if child.process.send_line(payload).is_err() {
            ...
            return Err(AppCoreError::Internal("slab-js-runtime stdin is closed".to_owned()));
        }
        ...
        let value = match result {
            ...
            Ok(Ok(Err(error))) => {
                return Err(AppCoreError::BadRequest(format!("JS plugin runtime error: {error}")));
            }
            Ok(Err(_)) => {
                return Err(AppCoreError::Internal(
                    "JS plugin runtime response channel closed".to_owned(),
                ));
            }
            Err(_) => {
                child.pending.lock().await.remove(&key);
                return Err(AppCoreError::Internal(format!(
                    "JS plugin runtime call timed out after {}ms",
        ...
        serde_json::from_value(value).map_err(|error| {
            AppCoreError::Internal(format!("invalid JS plugin runtime response: {error}"))
        })
    }
...
async fn spawn_runtime_child(
    ...
    let exit_handler = Arc::new(move |exit: SupervisedProcessExit| {
        ...
        Box::pin(async move {
            let message = exit.error.unwrap_or_else(|| {
                exit.status
                    .map(|status| format!("slab-js-runtime exited with {status}"))
                    .unwrap_or_else(|| "slab-js-runtime exited".to_owned())
            });
    ...
    let process = SupervisedStdioProcess::spawn(
        SupervisedStdioProcessConfig {
            label: "slab-js-runtime".to_owned(),
...
async fn read_child_stdout_line(
    ...
    let incoming = match serde_json::from_str::<JsonRpcIncoming>(&line) {
        Ok(value) => value,
        Err(error) => {
            tracing::warn!(%error, "invalid JSON-RPC line from slab-js-runtime");
            return;
        }
    };
    ...
    if let Some(method) = incoming.method.clone() {
        if method == "runtime.ready" {
            tracing::info!("slab-js-runtime reported ready");
            return;
        }
...
async fn handle_runtime_host_request(
    inner: &PluginJsRuntimeClientInner,
    method: &str,
    params: Value,
) -> Result<Value, String> {
    match method {
        ...
        _ => Err(format!("unknown slab-js-runtime host method `{method}`")),
    }
}
