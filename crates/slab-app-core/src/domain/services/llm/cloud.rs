//! Cloud LLM backend: catalog resolution + genai invocation wrapper.
//!
//! Resolves the requested model id into a genai target ([`ResolvedCloudModel`]) and wraps
//! genai non-streaming / streaming chat calls (including HTTP tracing and sensitive-field
//! redaction). chat and agent/response share cloud access through this module to avoid
//! duplicating genai client construction, request mapping, and HTTP diagnostics.
//!
//! Exposed surface:
//! - [`resolve_cloud_model`] / [`ResolvedCloudModel`]: catalog resolution (with credentials).
//! - [`cloud_chat_completion`] / [`cloud_chat_stream`]: genai non-streaming / streaming
//!   invocation, returning a neutral [`TextGenerationResponse`] / [`CloudTokenStream`] for
...
/// Resolved cloud invocation target: provider metadata + remote model id + credentials +
/// genai adapter.
#[derive(Debug, Clone)]
pub(crate) struct ResolvedCloudModel {
...
/// Resolve the requested cloud model id into an invocation target. Returns `BadRequest`
/// when the model is absent from the catalog.
pub(crate) async fn resolve_cloud_model(
    state: &ModelState,
    requested_model: &str,
...
pub(crate) fn is_cloud_catalog_model(model: &UnifiedModel) -> bool {
...
// ============ cloud chat request config + stream types ============
...
pub(crate) type CloudTokenStream =
    std::pin::Pin<Box<dyn futures::Stream<Item = Result<CloudDelta, AppCoreError>> + Send>>;
...
/// Non-streaming cloud chat completion; returns a neutral [`TextGenerationResponse`].
pub(crate) async fn cloud_chat_completion(
...
/// Streaming cloud chat completion; returns a neutral [`CloudTokenStream`].
pub(crate) async fn cloud_chat_stream(
    target: &ResolvedCloudModel,
    messages: &[DomainConversationMessage],
    config: CloudChatRequestConfig,
    trace_http: bool,
) -> Result<CloudTokenStream, AppCoreError> {
    info!(
        provider_id = %target.provider_id,
        provider_name = %target.provider_name,
        remote_model = %target.remote_model,
...
fn map_genai_error(action: &str, err: genai::Error) -> AppCoreError {
    ...
    {
        return AppCoreError::BadRequest(format!("cloud {action} failed: {detail}"));
    }
