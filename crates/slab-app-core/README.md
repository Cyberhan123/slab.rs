...
Plugin runtime gateway/client logic also lives here. `PluginService` dispatches JS and Python plugin calls to the supervised `bin/slab-js-runtime` and `bin/slab-python-runtime` sidecars over JSON-RPC, while `crates/slab-plugin` remains registry/WASM/frontend focused and does not depend on sidecar implementation details.
