...
use crate::openai::{
    ChatCompletionDeleted, CompactResource, CreateChatCompletionResponse,
    CreateChatCompletionRequest,
    CreateChatCompletionStreamResponse, CreateCompletionRequest, CreateCompletionResponse,
    CreateCompletionRequestModel,
    CreateEmbeddingRequest, CreateEmbeddingResponse, CreateTranscription200Response,
    ...
    ResponseCreatedEvent, ResponseErrorEvent, ResponseFailedEvent, ResponseInProgressEvent,
    ResponseFunctionCallArgumentsDeltaEvent, ResponseFunctionCallArgumentsDoneEvent,
    ResponseIncompleteEvent, ResponseItemList, ResponseQueuedEvent, ResponseStatus,
    ResponseTextDeltaEvent, ResponseTextDoneEvent,
    SkillListResource, SkillResource, SkillVersionListResource, SkillVersionResource,
    TranscriptTextDeltaEvent, TranscriptTextDoneEvent, TranscriptTextSegmentEvent,
    UpdateChatCompletionRequest, VideoCharacterResource, VideoContentVariant, VideoListResource,
    VideoResource,
};
...
const MODELS_LIST_RESPONSE: &str =
    r#"{"object":"list","data":[{"id":"string","created":0,"object":"model","owned_by":"string"}]}"#;
const MODEL_RESPONSE: &str =
    r#"{"id":"string","created":0,"object":"model","owned_by":"string"}"#;
...
const VOICE_CONSENT_RESPONSE: &str =
    r#"{"object":"audio.voice_consent","id":"cons_1234","name":"string","language":"string","created_at":0}"#;
...
const COMPLETIONS_REQUEST_CUSTOM_MODEL: &str =
    r#"{"model":"my-custom-model","prompt":"test"}"#;
...
const EMBEDDINGS_RESPONSE: &str =
    r#"{"data":[{"index":0,"embedding":[-3.4028236692093846E+38],"object":"embedding"}],"model":"string","object":"list","usage":{"prompt_tokens":0,"total_tokens":0}}"#;
const EMBEDDINGS_REQUEST: &str =
    r#"{"input":"hello","model":"text-embedding-3-small"}"#;
...
const VIDEO_CHARACTER_RESPONSE: &str =
    r#"{"id":"char_001","name":"Ava","created_at":0}"#;
...
const RESPONSE_INPUT_TOKENS: &str =
    r#"{"object":"response.input_tokens","input_tokens":123}"#;
...
const SKILL_RESPONSE: &str =
    r#"{"id":"string","object":"skill","name":"string","description":"string","created_at":0,"default_version":"string","latest_version":"string"}"#;
const SKILL_DELETE_RESPONSE: &str =
    r#"{"object":"skill.deleted","deleted":true,"id":"string"}"#;
...
const SKILL_VERSION_RESPONSE: &str =
    r#"{"object":"skill.version","id":"string","skill_id":"string","version":"string","created_at":0,"name":"string","description":"string"}"#;
...
const AUDIO_SPEECH_DELTA_EVENT: &str =
    r#"{"type":"speech.audio.delta","audio":"c3RyaW5n"}"#;
...
const AUDIO_TRANSCRIPT_TEXT_DONE_EVENT: &str =
    r#"{"type":"transcript.text.done","text":"hello world","usage":{"type":"tokens","input_tokens":12,"output_tokens":4,"total_tokens":16}}"#;
const AUDIO_TRANSCRIPT_TEXT_SEGMENT_EVENT: &str =
    r#"{"type":"transcript.text.segment","id":"seg_1","start":0.0,"end":1.2,"text":"hello","speaker":"A"}"#;
const RESPONSE_QUEUED_EVENT: &str =
...
const RESPONSE_IN_PROGRESS_EVENT: &str =
...
const RESPONSE_INCOMPLETE_EVENT: &str =
...
const RESPONSE_FAILED_EVENT: &str =
...
const RESPONSE_OUTPUT_TEXT_DELTA_EVENT: &str =
    r#"{"type":"response.output_text.delta","item_id":"msg_1","output_index":0,"content_index":0,"delta":"hel","sequence_number":8,"logprobs":[]}"#;
const RESPONSE_OUTPUT_TEXT_DONE_EVENT: &str =
    r#"{"type":"response.output_text.done","item_id":"msg_1","output_index":0,"content_index":0,"text":"hello","sequence_number":9,"logprobs":[]}"#;
const RESPONSE_FUNCTION_CALL_ARGS_DELTA_EVENT: &str =
    r#"{"type":"response.function_call_arguments.delta","item_id":"fc_1","output_index":0,"sequence_number":10,"delta":"{\"city\":\"Sh\"}"}"#;
const RESPONSE_FUNCTION_CALL_ARGS_DONE_EVENT: &str =
    r#"{"type":"response.function_call_arguments.done","item_id":"fc_1","name":"get_weather","output_index":0,"sequence_number":11,"arguments":"{\"city\":\"Shanghai\"}"}"#;
...
where
    T: DeserializeOwned,
{
    serde_json::from_str(fixture).unwrap_or_else(|error| panic!("failed to deserialize fixture: {error}\n{fixture}"))
}
