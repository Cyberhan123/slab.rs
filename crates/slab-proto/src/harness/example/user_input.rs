...
impl TextElement {
    ...
    {
        Self { byte_range: map(self.byte_range), placeholder: self.placeholder.clone() }
    }
    ...
    pub fn placeholder<'a>(&'a self, text: &'a str) -> Option<&'a str> {
        self.placeholder.as_deref().or_else(|| text.get(self.byte_range.start..self.byte_range.end))
    }
}
