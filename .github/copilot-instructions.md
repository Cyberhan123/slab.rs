...
- Do not assume a local slab.rs skill wrapper layer exists. Use the codebase directly unless the task clearly matches one of the real local skills:
  ...
  - `shadcn-ui`
  - `tauri-v2`
- `bin/slab-app` is the Tauri 2 desktop host that launches `bin/slab-server` as a sidecar and mounts local plugin webviews. Its frontend is `packages/slab-desktop`. It also embeds `crates/slab-app-core` (with the `tauri` feature) for native IPC commands.
...
- `crates/slab-app-core` (directory: `crates/slab-app-core/`) holds the HTTP-free business logic: `context/`, `domain/`, `infra/`, `config`, `model_auto_unload`. Use the `tauri` feature for `slab_app_core::tauri_bridge::register()`. Migrations live in `crates/slab-app-core/migrations/`.
