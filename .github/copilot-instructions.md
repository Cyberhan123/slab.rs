...
- The default third-party plugin UI model is a sandboxed Tauri child WebView, not Module Federation. Use `@slab/plugin-ui` and `@slab/plugin-sdk` for plugin UIs, and run `bun run build:plugins` from the repo root to build local plugin assets and refresh integrity hashes.
