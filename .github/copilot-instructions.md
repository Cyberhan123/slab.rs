...
- Do not assume a local slab.rs skill wrapper layer exists. Use the codebase directly unless the task clearly matches one of the real local skills:
  ...
  - `shadcn-ui`
  - `tauri-v2`
- `bin/slab-app` is the Tauri 2 desktop host that mounts local plugin webviews and calls `crates/slab-app-core` directly from native IPC commands in `bin/slab-app/src-tauri/src/api`. Desktop mode launches `bin/slab-runtime` through the shared `crates/slab-app-core::runtime_supervisor` using a Tauri sidecar adapter, not through `bin/slab-server`.
...
- `bin/slab-runtime` supports TCP or IPC transport and is the only runtime composition root. It uses an in-package `config` / `context` / `domain` / `infra` / `launch` layout and keeps GGML, Candle, and ONNX backends under `bin/slab-runtime/src/infra/backends`.
