...
- Windows 平台还需要 FFmpeg 开发库（原生链接 FFmpeg 使用），例如：`vcpkg install ffmpeg:x64-windows-static`（与仓库 `+crt-static` 的 MSVC 构建配置匹配）。
...
下面这些命令适合做常规构建、检查和测试。仓库入口统一使用 Bun 和 Cargo：
根 `package.json` 提供常用脚本，Rust 工作区由 Cargo 直接检查、测试和构建。
