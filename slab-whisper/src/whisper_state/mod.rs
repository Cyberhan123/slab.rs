...
impl WhisperState {
    ...
    pub fn pcm_to_mel(&mut self, pcm: &[f32], threads: usize) -> Result<(), WhisperError> {
        ...
        let ret = unsafe {
               self.ctx.instance.lib.whisper_pcm_to_mel_with_state(
                self.ctx.ctx,
                self.ptr,
                pcm.as_ptr(),
                pcm.len() as c_int,
    ...
    pub fn set_mel(&mut self, data: &[f32]) -> Result<(), WhisperError> {
        ...
        let ret = unsafe {
            self.ctx.instance.lib.whisper_set_mel_with_state(
                self.ctx.ctx,
                self.ptr,
                data.as_ptr(),
                n_len as c_int,
    ...
    pub fn encode(&mut self, offset: usize, threads: usize) -> Result<(), WhisperError> {
        ...
        let ret = unsafe {
            self.ctx.instance.lib.whisper_encode_with_state(
                self.ctx.ctx,
                self.ptr,
                offset as c_int,
                threads as c_int,
    ...
    pub fn decode(
        ...
        let ret = unsafe {
            self.ctx.instance.lib.whisper_decode_with_state(
                self.ctx.ctx,
                self.ptr,
                tokens.as_ptr(),
                tokens.len() as c_int,
    ...
    pub fn lang_detect(
        ...
        let mut lang_probs: Vec<f32> = vec![0.0; self.ctx.instance.get_lang_max_id() as usize + 1];
        let ret = unsafe {
            self.ctx.instance.lib.whisper_lang_auto_detect_with_state(
                self.ctx.ctx,
                self.ptr,
                offset_ms as c_int,
                threads as c_int,
    ...
    pub fn n_len(&self) -> c_int {
        unsafe { self.ctx.instance.lib.whisper_n_len_from_state(self.ptr) }
    }
    ...
    pub fn n_vocab(&self) -> c_int {
        unsafe { self.ctx.instance.lib.whisper_n_vocab(self.ctx.ctx) }
    }
    ...
    pub fn get_segment(&self, segment: c_int) -> Option<WhisperSegment<'_>> {
        self.segment_in_bounds(segment)
            .then(|| unsafe { self.ctx.instance.new_unchecked_segment(self, segment) })
    }
