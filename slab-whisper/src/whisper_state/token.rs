...
impl Whisper {
    pub(crate) unsafe fn new_unchecked_token<'a, 'b>(
        &self,
        segment: &'a WhisperSegment<'b>,
        token_idx: c_int,
    ) -> WhisperToken<'a, 'b> {
...
impl<'a, 'b> WhisperToken<'a, 'b> {
    ...
    pub fn token_id(&self) -> WhisperTokenId {
        unsafe {
            self.instance.lib.whisper_full_get_token_id_from_state(
    ...
    pub fn token_data(&self) -> WhisperTokenData {
        unsafe {
            self.instance.lib.whisper_full_get_token_data_from_state(
                self.segment.get_state().ptr,
                self.segment.segment_index(),
                self.token_idx,
            )
    ...
    pub fn token_probability(&self) -> f32 {
        unsafe {
            self.instance.lib.whisper_full_get_token_p_from_state(
                self.segment.get_state().ptr,
                self.segment.segment_index(),
                self.token_idx,
            )
    ...
    fn to_raw_cstr(&self) -> Result<&'b CStr, WhisperError> {
        let ret = unsafe {
            self.instance.lib.whisper_full_get_token_text_from_state(
