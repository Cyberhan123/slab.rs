...
impl Whisper {
    ...
    pub fn get_lang_max_id(&self) -> i32 {
        unsafe { self.lib.whisper_lang_max_id() }
    }
    ...
    pub fn get_lang_str(&self, id: i32) -> Option<&'static str> {
        let c_buf = unsafe { self.lib.whisper_lang_str(id) };
        if c_buf.is_null() {
            None
        } else {
    ...
    pub fn get_lang_str_full(&self, id: i32) -> Option<&'static str> {
        let c_buf = unsafe { self.lib.whisper_lang_str_full(id) };
        if c_buf.is_null() {
            None
        } else {
            let c_str = unsafe { CStr::from_ptr(c_buf) };
    ...
    pub unsafe fn set_log_callback(
        ...
        unsafe {
            self.lib.whisper_log_set(log_callback, user_data);
        }
    }
    ...
    pub fn print_system_info(&self) -> &'static str {
        let c_buf = unsafe { self.lib.whisper_print_system_info() };
        let c_str = unsafe { CStr::from_ptr(c_buf) };
        c_str.to_str().unwrap()
    }
}
