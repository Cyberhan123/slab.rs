...
fn normalize_runtime_control_path(path: Path) -> Path {
    if path.leading_colon.is_none() && path.segments.len() == 1 {
        let ident = path.segments[0].ident.clone();
        parse_quote!(crate::runtime::backend::protocol::RuntimeControlSignal::#ident)
    } else {
        path
    }
}
...
fn normalize_peer_control_path(path: Path) -> Path {
    if path.leading_colon.is_none() && path.segments.len() == 1 {
        let ident = path.segments[0].ident.clone();
        parse_quote!(crate::runtime::backend::protocol::PeerWorkerCommand::#ident)
    } else {
        path
    }
}
...
fn expand_backend_handler(item: TokenStream) -> Result<TokenStream> {
    ...
    let event_generated = event_routes.iter().enumerate().map(|(idx, route)| {
        ...
        quote! {
            ...
            fn #caller<'a>(
                &'a mut self,
                req: crate::runtime::backend::protocol::BackendRequest,
            ) -> crate::runtime::backend::runner::HandlerFuture<'a> {
                Box::pin(async move {
    ...
    let event_table_entries = (0..event_routes.len()).map(|idx| {
        ...
        let caller = format_ident!("__backend_handler_call_event_{}", idx);
        quote! {
            crate::runtime::backend::runner::EventRoute {
    ...
    let runtime_generated = runtime_routes.iter().enumerate().map(|(idx, route)| {
        ...
        quote! {
            fn #matcher(sig: &crate::runtime::backend::protocol::RuntimeControlSignal) -> bool {
                matches!(sig, #pattern { .. })
            }
            fn #caller<'a>(
                &'a mut self,
                signal: crate::runtime::backend::protocol::RuntimeControlSignal,
            ) -> crate::runtime::backend::runner::HandlerFuture<'a> {
                Box::pin(async move {
                    #call
                })
    ...
    let runtime_table_entries = (0..runtime_routes.len()).map(|idx| {
        ...
        let caller = format_ident!("__backend_handler_call_runtime_{}", idx);
        quote! {
            crate::runtime::backend::runner::RuntimeRoute {
                matches: Self::#matcher,
                handle: Self::#caller,
            }
        }
    ...
    let peer_variant_generated = peer_routes.iter().enumerate().map(|(idx, route)| {
        ...
        quote! {
            fn #matcher(cmd: &crate::runtime::backend::protocol::PeerWorkerCommand) -> bool {
                matches!(cmd, #pattern { .. })
            }
            fn #caller<'a>(
                &'a mut self,
                cmd: crate::runtime::backend::protocol::PeerWorkerCommand,
            ) -> crate::runtime::backend::runner::HandlerFuture<'a> {
                Box::pin(async move {
                    #call
                })
    ...
    let peer_table_entries = (0..peer_routes.len()).map(|idx| {
        ...
        let caller = format_ident!("__backend_handler_call_peer_route_{}", idx);
        quote! {
            crate::runtime::backend::runner::PeerRoute {
                matches: Self::#matcher,
                handle: Self::#caller,
            }
        }
    ...
    let peer_fallback_route = if peer_fallback_method.is_some() {
        quote! { Some(Self::__backend_handler_call_peer as crate::runtime::backend::runner::PeerDispatchFn<Self>) }
    } else {
        quote! { None }
    };
    ...
    let lagged_route = if control_lagged_method.is_some() {
        quote! { Some(Self::__backend_handler_call_lagged as crate::runtime::backend::runner::LaggedDispatchFn<Self>) }
    } else {
        quote! { None }
    };
    ...
    let expanded = quote! {
        ...
        impl #impl_generics crate::runtime::backend::runner::RuntimeWorkerHandler for #self_ty #where_clause {
            async fn handle_request(&mut self, req: crate::runtime::backend::protocol::BackendRequest) {
                crate::runtime::backend::runner::dispatch_backend_request(
                    self,
                    req,
                    &[#(#event_table_entries),*],
                ).await;
            ...
            async fn handle_peer_control(
                ...
                crate::runtime::backend::runner::dispatch_peer_control(
                    self,
                    cmd,
                    #peer_fallback_route,
                    &[#(#peer_table_entries),*],
            ...
            async fn handle_runtime_control(
                ...
                crate::runtime::backend::runner::dispatch_runtime_control(
                    self,
                    signal,
            ...
            async fn handle_control_lagged(&mut self) {
                crate::runtime::backend::runner::dispatch_control_lagged(self, #lagged_route).await;
            }
        }
    };
