    ...
    pub mod execution {
        pub mod backend {
            ...
            pub mod runner {
                ...
                use super::protocol::{
                    BackendRequest, PeerWorkerCommand, RequestRoute, RuntimeControlSignal,
                };

                pub type HandlerFuture<'a> = Pin<Box<dyn Future<Output = ()> + Send + 'a>>;
                pub type RequestDispatchFn<T> =
                    for<'a> fn(&'a mut T, BackendRequest) -> HandlerFuture<'a>;
                pub type RuntimeDispatchFn<T> =
                    for<'a> fn(&'a mut T, RuntimeControlSignal) -> HandlerFuture<'a>;
                pub type PeerDispatchFn<T> =
                    for<'a> fn(&'a mut T, PeerWorkerCommand) -> HandlerFuture<'a>;
...
use crate::internal::execution::backend::protocol::{
    BackendRequest, PeerWorkerCommand, RuntimeControlSignal,
};
