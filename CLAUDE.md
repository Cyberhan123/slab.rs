...
- `bin/slab-runtime` serves gRPC over TCP or IPC and is the only runtime composition root. It now uses an in-package `config` / `context` / `domain` / `infra` / `launch` layout and keeps GGML, Candle, and ONNX backend implementations under `bin/slab-runtime/src/infra/backends`.
