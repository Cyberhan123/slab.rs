...
export function AudioTranscriptDetail({ isTauri, task }: AudioTranscriptDetailProps) {
  ...
  const sourceUrl = useMemo(
    () => resolveAudioSource(task.source_path, isTauri, ports.imageSrc),
    [isTauri, task.source_path, ports.imageSrc],
  );
...
function resolveAudioSource(
  path: string,
  isTauri: boolean,
  imageSrc: { canResolveLocalPaths(): boolean; resolve(pathOrUrl: string): string },
) {
  if (!path.trim()) {
    return null;
  }
  ...
  return isTauri && imageSrc.canResolveLocalPaths() ? imageSrc.resolve(path) : null;
}
