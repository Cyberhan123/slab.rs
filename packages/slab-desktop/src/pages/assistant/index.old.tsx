...
} from "./components/assistant-bubble-content"
...
function Assistant() {
  ...
  const toAssistantBubbleContent = useCallback(
    (item: AssistantMessageRecord): AssistantBubbleContent => ({
    ...
    <div className="relative flex min-h-0 flex-1 flex-col bg-[var(--shell-card)]">
      ...
      <div className="min-h-0 flex-1">
        <MessageScrollerProvider autoScroll>
          <MessageScroller>
            <MessageScrollerViewport>
              <MessageScrollerContent className="mx-auto w-full max-w-[682px] px-6 pb-24 pt-2 md:px-8 md:pb-28 lg:px-0">
                {isSessionBootstrapping || (isHistoryLoading && safeMessages.length === 0) ? (
                  <MessageScrollerItem messageId="assistant-loading-state">
                    <Empty
                      className="min-h-[260px] border border-dashed border-border/60 bg-[var(--surface-soft)]"
                  ...
                  <MessageScrollerItem messageId="assistant-empty-state">
                    <Empty
                      className="min-h-[260px] border border-dashed border-border/60 bg-[var(--surface-soft)]"
                  ...
                  safeMessages.map((item) => {
                    const content = toAssistantBubbleContent(item)
                    const role = item.message.role === "assistant" ? "assistant" : "user"
                    const align = role === "assistant" ? "start" : "end"
                    ...
                    return (
                      ...
                      >
                        <Message align={align}>
                          <MessageAvatar>
                            <AssistantMessageAvatar role={role} />
                          </MessageAvatar>
                          <MessageContent>
                            <MessageHeader>{getAssistantMessageLabel(content)}</MessageHeader>
                            <Bubble
                              align={align}
                              variant="ghost"
                ...
                {isPreparingModel || modelLoading ? (
                  <MessageScrollerItem messageId="assistant-model-loading">
