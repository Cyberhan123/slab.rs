import { describe, expect, it } from 'vitest'

import { toStoredSessionChatMessage } from '../chat-history'

describe('chat history', () => {
  it('keeps plain stored content when it is not JSON', () => {
    expect(toStoredSessionChatMessage(' user ', 'plain text')).toEqual({
      role: 'user',
      content: 'plain text',
    })
  })

  it('renders stored message envelopes with content parts and tool calls', () => {
    const stored = {
      message: {
        role: ' assistant ',
        content: [
          { type: 'text', text: 'hello' },
          { type: 'json', value: { ok: true } },
          { type: 'tool_result', tool_call_id: 'call-1', value: { value: 42 } },
        ],
        tool_call_id: 'call-1',
        tool_calls: [
          {
            id: 'call-2',
            function: {
              name: 'lookup',
              arguments: { query: 'slab' },
            },
          },
        ],
      },
    }

    expect(toStoredSessionChatMessage('user', JSON.stringify(stored))).toEqual({
      role: 'assistant',
      content: [
        'hello',
        '{"ok":true}',
        'tool_result[call-1]: {"value":42}',
        'tool_call_id: call-1',
        'tool_call id=call-2: lookup({"query":"slab"})',
      ].join('\n'),
    })
  })

  it('falls back to the response role when a stored message has no role', () => {
    expect(
      toStoredSessionChatMessage(
        ' assistant ',
        JSON.stringify({
          content: { text: 'from storage' },
        })
      )
    ).toEqual({
      role: 'assistant',
      content: 'from storage',
    })
  })

  it('preserves unknown stored roles after trimming', () => {
    expect(
      toStoredSessionChatMessage(
        'user',
        JSON.stringify({
          role: ' critic ',
          content: 'review',
        })
      )
    ).toEqual({
      role: 'critic',
      content: 'review',
    })
  })
})
