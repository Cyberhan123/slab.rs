...
- `tests/manual/*.test.ts`: opt-in diagnostics for real local-dev environments. These are not part of the root E2E command; run one explicitly with `bunx vitest run --config packages/slab-desktop/vitest.manual.config.ts <name-filter>` (each file gates itself on its own env vars, e.g. `SLAB_E2E_REAL_MODEL=1` for the real-model thinking-budget verification).
